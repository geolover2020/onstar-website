...
export default function Guide(){
 ...
 return <>
  ...
  <section className="section guide-section"><div className="container">
   ...
   {cat==='all'&&!query && <div className="category-overview">{toolCategories.map(c=><article key={c.id} onClick={()=>setCat(c.id)}><div className="cat-icon"><Icon name={c.icon}/></div><div><h3>{c.title}</h3><p>{c.description}</p><small>{tools.filter(t=>t.category===c.id).length} أداة</small></div></article>)}</div>}
   <div className="tools-grid">{filtered.map(t=>{const c=cOf(t.category);return <article className="tool-card" key={t.slug} onClick={()=>openTool(t)}><div className="tool-card-top"><div className="cat-icon small"><Icon name={c?.icon}/></div><span>{c?.title}</span></div><h3>{t.title}</h3><p>{t.summary}</p><div className="tool-tags">{(t.tags||[]).slice(0,3).map(x=><span key={x}>{x}</span>)}</div><button>فتح الشرح <Icon name="arrow" size={17}/></button></article>})}</div>
