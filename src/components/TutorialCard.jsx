...
export default function TutorialCard({ item, category }) {
  ...
  return <article className="tutorial-card">
    ...
    <div className="tutorial-body"><small>{category?.icon} {category?.title}</small><h3>{item.title}</h3><a href={url} target="_blank" rel="noreferrer">مشاهدة الشرح <span>↗</span></a></div>
  </article>
}
